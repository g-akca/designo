...
function StorySection({ heading, story, image, tabletImage, desktopImage, isReversed = false }: StorySectionProps) {
  ...
  return (
    <div 
      ...
      `}
    >
      <picture className="desktop:w-119 desktop:shrink-0">
        <source media="(min-width: 1440px)" srcSet={desktopImage} />
        <source media="(min-width: 768px)" srcSet={tabletImage} />
        <img src={image} alt="" className="h-80 w-full object-cover desktop:h-full" />
      </picture>
      ...
      <div 
        className={`
          ...
          desktop:min-h-auto ${isReversed ? "desktop:pl-23.75" : "desktop:pr-24"}
        `}
      >
